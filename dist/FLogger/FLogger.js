...
class FLogger {
     ...
     * */
    constructor(options = { writeToText: false, timestamps: false, path: "./", color: "white", logWrite: false, warnings: false }) {
        /**
         * Options:
         * Write to a .txt file.
         * timestamps.
        ...
        if (this.writeToText) {
            ...
            if (this.warnings) {
                if (this.timestamps) {
                    ...
                    console.log(`\x1b[93m\u2588\u2588\u2588 [WRN] > Property: writeToText must be set to true. Cannot write to ${this.fileName}.\x1b[0m`);
            }
        }
    }
    /**
    ...
    _internalWrite(str) {
        if (this.timestamps)
            this.write(str.slice(20));
        else
            this.write(str.slice(9));
        str += `\x1b[0m`;
        console.log(str);
    }
    log(type = { property: undefined, filePath: undefined, bounds: undefined }, str, highlight) {
        ...
        output += "\u2588\u2588\u2588 ";
        if (typeof type.property === "undefined")
            property = false; // Defaults to false if undefined.
        else
            property = type.property;
        if (typeof type.filePath === "undefined")
            filePath = false;
        else
            filePath = type.filePath;
        if (typeof type.bounds === "undefined")
            bounds = false;
        else
            bounds = type.bounds;
        if (this.timestamps) {
    ...
    logError(type = { property: {}, filePath: undefined, bounds: undefined }, str, highlight) {
        var property, filePath, bounds;
        var output = "";
        if (highlight)
            output += "\x1b[30;41m";
