export class FConsoleRead {
    constructor(options: { commands: { commandNameString?: Function }; exit: { command?: string; exitFunction?: Function; }; }, onceEnable: boolean) {
        ...
        stdin.on("data", data => {
            ...
            let readArr: string[] = Array.from(send);             // New array from send
            ...
            if (!Object.keys(options.commands).includes(command) && cmdCheck) {
                ...
                for (let i = 0; i < readArr.length; i++) {
                    if (readArr[i] === "-string") {
                        ...
                        argsArr.push(parseInt(sendArr.reduce((p: string, n: string) => parseInt(p) + parseInt(n))));
                        continue;
                    }
        ...
        stdin.on('close', _c => {
            let exitFunc: any = options.exit.exitFunction;
            let param = true;
            exitFunc(param);
